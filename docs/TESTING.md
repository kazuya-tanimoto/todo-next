...
ローカルSupabase + 実DBに対して中核フロー（Todo CRUD / リスト管理）と
共有フロー（招待→参加、共有Todoの相互可視）を2セッションで検証する。
CI/Docker化のみ次段に分離。
...
- `e2e/auth.setup.ts`: 認証セットアップ（owner/member の2ユーザー分。後述）
- `e2e/helpers/`: `supabase-admin.ts`（service_roleクライアント + 招待トークン取得）、`auth-cookie.ts`（cookie採取）、`ui.ts`（UI操作ヘルパ）
- `e2e/{todo-crud,list-management}.spec.ts`: 中核フローのテスト本体
- `e2e/sharing.spec.ts`: 共有フロー（owner + member の2 storageState を別 context で使い分け）
...
6. `context.addCookies`（url=`http://localhost:3000`）→ storageState 保存

手組みではなく `createServerClient` に cookie を生成させるのが要点。base64・チャンク分割の仕様差にハードコードで依存しない。

owner（`user.json`）と member（`user-b.json`）の2ユーザー分を同じ手順で seed する。
member メールは未指定なら owner メールから `<local>-b@<domain>` を導出するため、追加の env なしで2ユーザーを賄える。
共有フローの招待トークンはクリップボードに頼らず service_role で `invite_tokens` から取得する（ヘッドレスで安定）。
...
- `E2E_TEST_EMAIL_B` / `E2E_TEST_PASSWORD_B`（任意。未指定なら owner メールから `<local>-b@<domain>` を導出）
...
- テストユーザー（owner）を共有するため直列実行。各テストは一意名リストを作って自己完結させる。共有フローは別ユーザー（member）の storageState で `browser.newContext()` を起こして2セッションにする
- 共有Todoの owner 側反映は Realtime に頼らず、`page.reload()` + リスト再選択で再fetchして決定的に確認する
- Todo削除・一括削除に確認ダイアログは無いが、リスト削除は `window.confirm` が出る → 削除系テストは操作前に `page.on('dialog', d => d.accept())` を登録する

#### CI（GitHub Actions）

`.github/workflows/ci.yml` で push / PR ごとに2 jobを回す。

- `quality`（Docker不要）: `yarn check:ci`（Biome）→ `yarn typecheck` → `yarn typecheck:e2e` → `yarn test`（unit）
- `e2e`（Docker使用）: `supabase start`（migrations適用）→ `supabase status -o env` で anon/service_role を取得 → `yarn playwright install --with-deps chromium` → `yarn test:e2e`

要点:
- Node 22 LTS（Next 16の `>=20.9` を満たし、unitの `--no-experimental-webstorage` が有効な 22.4+ を含む）
- yarnは `corepack enable` で `packageManager: yarn@4.12.0` を有効化、`yarn install --immutable`
- `NEXT_PUBLIC_SUPABASE_URL` は `http://127.0.0.1:54321` 固定（認証cookie名 `sb-127-auth-token` が URL host 由来のため）。anon/service_role はハードコードせず `supabase status` から取得し、キー名が取れなければ生出力をダンプして明示的に失敗させる
- アプリ起動は `yarn dev`（playwright の `webServer`。`CI=true` で `reuseExistingServer=false` / `forbidOnly=true` が効く）
- 失敗時は `playwright-report/` と `test-results/` を artifact としてアップロード
- ローカル専用の docker-compose は用意していない（`supabase start` が内部でDockerを使うため、CIのE2Eで再現性は担保される）
