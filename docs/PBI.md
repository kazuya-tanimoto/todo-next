...
- **進捗メモ**:
  - 実装完了: マイグレーション、型、Supabase 更新ハンドラ、TodoItem 期限ピッカー UI、期限バッジ（overdue/soon/future の3状態、しきい値3日）、SortSelector「期限」オプション、ユニットテスト（87件 pass）
  - 未消化: ローカル E2E（MCP Playwright + ローカル Supabase）、本番デプロイ後の本番 E2E
  - 上記2工程を完了後に DONE へ更新する
