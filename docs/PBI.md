...
- **要件**:
  - 個別削除Todoを元リストごとに見出し付きでまとめる（`TrashView.tsx` の現状フラット表示 `:246-277` を置き換え）
  - 既存の「削除済みリスト」セクション（`TrashView.tsx:211-243`）と同じ見た目に揃える（リスト名見出し＋配下に項目）。ゴミ箱全体がリスト単位でまとまる一貫表示にする
  - 生存リストの名前を取得（現状は削除済みリスト名しか取得していない）
  - リスト名が長い場合は見出しで省略（`truncate`）し、スマホでも1行が破綻しないこと
