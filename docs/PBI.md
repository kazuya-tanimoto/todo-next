...
- **要件**:
  - [x] 個別削除Todoを元リストごとに見出し付きでまとめる（`TrashView.tsx` のフラット表示を `orphanGroups` 単位の表示へ置き換え）
  - [x] 既存の「削除済みリスト」セクションと同じ見た目に揃える（`theme-card` ＋ 太字リスト名見出し＋ `ml-4` 配下に項目）。ゴミ箱全体がリスト単位でまとまる一貫表示にした
...
- **メモ**: 実装＋ユニットテスト完了。`TrashView.test.tsx` にグルーピング検証テスト（同一リストのTodoは見出し1回＋複数リストの見出し並列）を追加し、unit 89件 green / `yarn typecheck` green / `yarn check:ci`（Biome）exit 0。ローカルE2Eはサンドボックスで Docker 不可のため未実行。残工程: デプロイ（push → Vercel）→ CI e2e job → 本番E2E。
...
- **要件**:
  - `TodoItem.tsx` のカード親 div に `group` クラスを付ける、または `.theme-delete` の常時表示に変更
  - リスト項目の Rename / Delete ボタンも同種の hover 表示パターンか確認し、同じ問題があれば併せて修正
